...
namespace Serilog.Sinks.BrowserHttp
{
    class BrowserHttpSink : PeriodicBatchingSink
    {
        ...
        {
            // Or, we could try to base this on window.location.origin... 🤔
            return new Uri(new MonoWebAssemblyJSRuntime().Invoke<string>("Blazor._internal.navigationManager.getBaseURI"));
        }
